...
10. 回写重写后的 HTML，并在单独事务中建立 `page_resources` 关联，同时把 `snapshot_state` 置为 `ready`
11. 同步创建若在 finalize 前失败，会回滚页面记录并删除临时 HTML；异步创建若 finalize 失败，则把页面标记为 `failed`，等待后续同 hash 捕获自动重试
