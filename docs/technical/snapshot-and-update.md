...
3. 先保存临时 HTML，并创建 `pages` 记录以获取 `pageID`
4. 从 HTML 中提取资源 URL（img/link/script 等）
5. 8 个 worker 并发下载/处理资源
6. 每个资源按哈希去重；即使 URL 相同，只要哈希变化也会创建新版本记录
7. 对 CSS：提取并下载其子资源，但不原地改写共享 CSS 文件
8. 用 `URLRewriter` 将 HTML 中的外部 URL 替换为本地路径
9. 回写重写后的 HTML，并在单独事务中建立 `page_resources` 关联
10. 若在页面创建后、最终关联前出现普通错误，会回滚页面记录并删除临时 HTML；若进程在此窗口内异常退出，仍可能留下未完成的页面记录
...
3. 通过单个事务更新 `html_path` / `content_hash` / `title`，同时替换 `page_resources` 关联
4. 事务成功后，旧 HTML 文件进入删除队列（保留 7 天）
5. 若更新中途失败，会删除新的临时 HTML，并保留旧页面内容和旧资源关联
6. 更新前会校验请求体 `url` 与 `page_id` 对应页面的 `url` 完全一致；不一致时直接拒绝，避免跨页面污染时间线、搜索索引和资源关联
...
- 浏览器侧会等待 `iframe` 达到 ready 状态；若最终仍只得到登录墙或明显空壳内容，则保留外层占位但不再回退抓取错误的原站子页面
